    ...
    if (followExist) {
      const newState = users?.map((cuser: any) => {
        if (cuser._id === user?._id) {
          return {
            ...cuser,
            following: cuser?.following?.filter((cuser1: any) => {
              return cuser1.id != currentUser._id;
            })
          };
        }
        if (cuser._id == currentUser._id) {
          return {
            ...cuser,
            followers: cuser?.followers?.filter((cuser1: any) => {
              return cuser1.id != user?._id;
            })
          };
        }
        return cuser;
      ...
      const newState = users?.map((cuser: any) => {
        if (cuser._id === user._id) {
          return {
            ...cuser,
            following: [...cuser.following, { id: currentUser._id, date }]
          };
        }
        if (cuser._id === currentUser._id) {
          return {
      ...
      setUserAccount((cuser: any) => {
        if (cuser._id == user._id) {
          return {
            ...cuser,
            following: [...cuser?.following, { id: user._id, date }]
          };
        }
        return {
          ...cuser,
    ...
    <>
      ...
      <div
        className={`lg:w-[80%] xl:w-[50%] m-auto h-full my-2 relative delay-100 ${
          showPopUp ? "blur-md opacity-75 pointer-events-none select-none" : ""
        }`}
      >
        <div className="border h-[20em] w-full bg-gray-200 rounded-md flex flex-col">
          <div className="border rounded-full h-1/2 relative">
                ...
                >
                  ...
                  >
                    <img
                      ...
                      className="w-12 rounded-full border-2 object-fit"
                    />
                      ...
                      {users
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            let date = new Date();
                            followBtn(poster(data.id, users), date);
                            follow(
                              { id: user?._id, date },
                              { id: data?.id, date }
                            );
